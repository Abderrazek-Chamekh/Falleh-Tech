...
public class AjouterProduitController {
    ...
    public void ajouterProduit() {
        ...
        if (nom.isEmpty() || nom.length() < 3) {
            showError(nomField, nomError, "⚠ Le nom doit contenir au moins 3 lettres.");
            return;
        }
        ...
        if (cat == null || sousCat == null) {
            showError(categorieError, "⚠ Catégorie et sous-catégorie requises.");
            return;
        }
    ...
    private void fermerPopupAvecDelai() {
        new Thread(() -> {
            try {
                Thread.sleep(1000);
