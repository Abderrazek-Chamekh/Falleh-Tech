...
public class RewardViewController implements Initializable {
    ...
    private void telechargerPDF(String filename) {
        try {
            File pdfSource = new File("src/main/resources/pdfs/" + filename);
            if (!pdfSource.exists()) {
                System.err.println("❌ Fichier PDF introuvable : " + pdfSource.getAbsolutePath());
                return;
            }
            ...
            if (destination != null) {
                Files.copy(pdfSource.toPath(), destination.toPath(), StandardCopyOption.REPLACE_EXISTING);
